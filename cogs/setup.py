...
class submit_key(discord.ui.Modal, title="Setup Ranking"):
    ...
    async def on_submit(self, interaction : discord.Interaction):
        ...
        if api_data.guild_already_setup == True:
            api_data.update_info(interaction.guild.icon, self.key.value)
        else:
...
class setup_class(commands.Cog):
    ...
    async def setup_logging(self, interaction : discord.Interaction, logging_channel_id : str):
        ...
        await interaction.response.defer()
        if config_data.guild_already_setup == True:
    ...
    @app_commands.checks.has_permissions(administrator=True)
    async def add_role(self, interaction : discord.Interaction, role_id : str):
        role_id = int(role_id)
        await interaction.response.defer()
        current_data = config_data.fetch_configs(interaction.guild.id)
        required_roles = current_data["required_roles"]
    ...
    @app_commands.checks.has_permissions(administrator=True)
    async def remove_role(self, interaction : discord.Interaction, role_id : str):
        role_id = str(role_id)
        await interaction.response.defer()
        current_data = config_data.fetch_configs(interaction.guild.id)
        required_roles = current_data["required_roles"]
